...
template <typename T>
void ListaCircular<T>::eliminarCaracter(NodoCircular<T>* lista, std::string caracter) {
    if (listaVacia()) {
        return;
    }

    NodoCircular<T>* actual = lista;
    NodoCircular<T>* inicio = lista;

    do {
        std::string cedula = actual->getNumCedula();
        cedula.erase(std::remove(cedula.begin(), cedula.end(), caracter[0]), cedula.end());
        actual->setNumCedula(cedula);
        actual = actual->getSiguiente();
    } while (actual != inicio);
...
template <typename T>
T ListaCircular<T>::ingresarCaracter(const std::string& msj) {
    T dato = "";
    cout << msj << ": ";
    char c;
    while ((c = getch()) != 13) {
        if (isdigit(c) && dato.empty()) {
            dato = c;
            cout << c;
        } else if (c == 8 && !dato.empty()) {
            dato = "";
