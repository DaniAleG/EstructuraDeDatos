...
template <typename T>
void ListaSimple<T>::eliminarCaracter(NodoSimple<T>* lista, string caracter) {
    std::function<void(NodoSimple<T>*, string)> eliminarRecursivo = [&](NodoSimple<T>* nodo, string c) {
        if (nodo == nullptr) 
        return; 
        std::string cedula = nodo->getNumCedula(); 
        cedula.erase(std::remove(cedula.begin(), cedula.end(), c[0]), cedula.end()); 
        nodo->setNumCedula(cedula);
        eliminarRecursivo(nodo->getSiguiente(), c); 
    };
    eliminarRecursivo(lista, caracter); 
    guardarEnArchivo();
    
}
template <typename T>
T ListaSimple<T>::ingresarCaracter(const std::string& msj) {
    T dato = "";
    cout << msj << ": ";
    char c;
    while ((c = getch()) != 13) {
