...
template <typename T>
void ListaDoble<T>::eliminarCaracter(NodoDoble<T>* lista, string caracter) {
    std::function<void(NodoDoble<T>*, string)> eliminarRecursivo = [&](NodoDoble<T>* nodo, string c) {
        if (nodo == nullptr) 
            return; 
...
template <typename T>
T ListaDoble<T>::ingresarCaracter(const std::string& msj) {
    T dato = "";
    cout << msj << ": ";
    char c;
    while ((c = getch()) != 13) {
